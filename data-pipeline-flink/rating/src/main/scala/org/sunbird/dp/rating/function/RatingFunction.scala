...
class RatingFunction(config: RatingConfig, @transient var cassandraUtil: CassandraUtil = null)(implicit val mapTypeInfo: TypeInformation[Event])
  ...
  override def processElement(event: Event, context: ProcessFunction[Event, Event]#Context, metrics: Metrics): Unit = {
    ...
    try {
      ...
      if (delta != 0 || (validReview.size > 100)) {
        ...
        //val ratingRows =getRatingSummary(event)


        val ratingRows: java.util.List[Row] = cassandraUtil.find(ratingQuery);
        ...
        if (null != ratingRows && !ratingRows.isEmpty) {
          ...
          if (validReview.size > 100 && delta == 0) {
            ...
            updatedRatingValues = update_ratings_count(tempRow, prevRating, 0.0f)
            counter = 1
  ...
  def updateDB(event: Event, updatedRatingValues: HashMap[Float, Float],
    ...
    if (null == ratingDBResult) {
      ...
      if (validReview.size > 100) {
        updatedReviews = update_Top50_Review_Summary(latest50Reviews, event)
  ...
  def update_ratings_count(tempRow: Row, prevRating: Float, updatedRating: Float): HashMap[Float, Float] = {
    val ratingMap: HashMap[Float, Float] = new HashMap()
    var x = 0;
    for (i <- 0 to 4) {
      ratingMap.put(i + 1, 0)
    }
    ...
    if (prevRating != 0.0f) {
      if (ratingMap.containsKey(oldRating) && newRating != oldRating && ratingMap.get(oldRating)!=0) {
  ...
  def update_Top50_Review_Summary(latest50Reviews: String, event: Event): String = {

    val gson: Gson = new Gson()
    val ratingQueue = mutable.Queue[RatingJson]()
    val updatedReviewSize = event.updatedValues.get("review").asInstanceOf[String].size
    if (updatedReviewSize > 100) {

      // Generate latest 50 reviews and add new review if its size is more than 100
      if (!latest50Reviews.isEmpty) {

        val ratingJson: Array[RatingJson] = gson.fromJson(latest50Reviews, classOf[Array[RatingJson]])
