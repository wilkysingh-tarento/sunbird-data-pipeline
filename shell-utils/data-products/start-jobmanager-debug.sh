...
kill_job_manager()
{
    ...
    kill $(ps aux | grep 'org.ekstep.analytics.job.JobManager' | awk '{print $2}') >> "$SERVICE_LOGS/$today-job-manager.log"
}
...
start_job_manager()
{
    ...
    job_manager_pid=$(ps aux | grep 'org.ekstep.analytics.job.JobManager' | awk '{print $2}') # Once Job is started just we are making whether job is running or not.
